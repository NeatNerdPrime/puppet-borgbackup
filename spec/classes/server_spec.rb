  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with defaults' do
        ...
        it {
          is_expected.to contain_user(params[:borguser])
            .with_ensure('present')
      ...
      context 'with not creating user' do
        ...
        it { is_expected.not_to contain_user(params[:borguser]) }
      end
      ...
      describe 'with authorized-keys' do
        ...
        it_behaves_like 'borgbackup::server shared examples'
        it {
          is_expected.to contain_borgbackup__authorized_key('firstkey')
      ...
      describe 'with another authorized-keys' do
        ...
        it {
          is_expected.to contain_file('/etc/borgbackup/.ssh/authorized-keys')
            .with_owner('myowner')
        }
      end
    end
